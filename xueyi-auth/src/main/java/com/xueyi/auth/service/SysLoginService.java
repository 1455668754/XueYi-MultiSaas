...
import com.xueyi.system.api.feign.RemoteLoginService;
...
public class SysLoginService {
    ...
    public LoginUser login(String enterpriseName, String userName, String password) {
        ...
        if (BaseConstants.DelFlag.DELETED.getCode().equals(user.getDelFlag())) {
            recordLoginInfo(userInfo.getMainSource(), enterprise.getId(), enterpriseName, user.getId(), userName, Constants.LOGIN_FAIL, "对不起，您的账号已被删除");
            throw new ServiceException("对不起，您的账号：" + userName + " 已被删除");
        }

        if (BaseConstants.Status.DISABLE.getCode().equals(user.getStatus())) {
            recordLoginInfo(userInfo.getMainSource(), enterprise.getId(), enterpriseName, user.getId(), userName, Constants.LOGIN_FAIL, "用户已停用，请联系管理员");
            throw new ServiceException("对不起，您的账号：" + userName + " 已停用");
        }

        if (!SecurityUtils.matchesPassword(password, user.getPassword())) {
            recordLoginInfo(userInfo.getMainSource(), enterprise.getId(), enterpriseName, user.getId(), userName, Constants.LOGIN_FAIL, "用户密码错误");
            throw new ServiceException("企业账号/员工账号/密码错误，请检查");
        }

        recordLoginInfo(userInfo.getMainSource(), enterprise.getId(), enterpriseName, user.getId(), userName, Constants.LOGIN_SUCCESS, "登录成功");
        return userInfo;
    }
